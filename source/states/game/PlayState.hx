...
class PlayState extends MusicBeatState
{
	...
	public static var songPosBar:FunkinBar;
	...
	override public function create()
	{
		...
		for (allowed in HScript.allowedExtensions)
		{
			...
			{
				for (scriptFile in FileSystem.readDirectory('./mods/global/scripts'))
				{
					if (scriptFile.contains(allowed))
					{
						var script = new HScript('assets/scripts/${scriptFile.replace("." + allowed, "")}');
			...
			default:
				{
					...
					for (allowed in HScript.allowedExtensions)
					{
						...
						{
							var script = new HScript('assets/stages/stage');
		...
		var dad = new Character(100, 100, SONG.player2);
		dad.x += dad.positions[0];
		dad.y += dad.positions[1];
		...
		var boyfriend = new Character(770, 450, SONG.player1, true);
		boyfriend.x += boyfriend.positions[0];
		boyfriend.y += boyfriend.positions[1];
		...
		for (script in scripts)
		{
			script.setValue('bf', boyfriendGroup.members[0]);
			script.setValue('boyfriend', boyfriendGroup.members[0]);
			script.setValue('dad', dadGroup.members[0]);
			script.setValue('gf', gfGroup.members[0]);
		}
		...
		FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / Main.fpsCounter.currentFPS));
		...
		healthBar = new FunkinBar(0, FlxG.height * 0.4 + 10, "healthBar", dad.iconColor, boyfriend.iconColor, RIGHT_TO_LEFT, 'health', 0, 2);
		healthBar.screenCenter();
		healthBar.scrollFactor.set();
		add(healthBar);

		// Add Kade Engine watermark
		kadeEngineWatermark = new FlxText(4, healthBar.bg.y + 50, 0, SONG.song + " " + storyDifficulty + (Main.watermarks ? " - KE+ 1.0.0 (PRE 1.0)" : ""), 16);
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();
		add(kadeEngineWatermark);

		replayTxt = new FlxText(healthBar.bg.x + healthBar.bg.width / 2 - 75, healthBar.bg.y + (FlxG.save.data.downscroll ? 100 : -100), 0, "REPLAY", 20);
		...
		iconP1 = new HealthIcon(boyfriend.iconImage, true);
		iconP1.y = healthBar.bar.y - (iconP1.height / 2);
		add(iconP1);

		iconP2 = new HealthIcon(dad.iconImage, false);
		iconP2.y = healthBar.bar.y - (iconP2.height / 2);
		add(iconP2);

		scoreTxt = new FlxText(0, healthBar.bg.y + 25, FlxG.width, "", 16);
		if (!FlxG.save.data.accuracyDisplay)
			scoreTxt.x = healthBar.bg.x + healthBar.bg.width / 2;
				...
				default:
					...
					if (hasCutsene)
					{
						...
						if (!script.isBlank && script.expr != null)
						{
							...
							script.setValue('dad', dadGroup.members[0]);
							script.setValue('gf', gfGroup.members[0]);
							script.setValue("isCutsene", true);
							script.interp.execute(script.expr);
						}
	...
	function startCountdown():Void
	{
		...
		{
			for (character in [dadGroup.members, gfGroup.members, boyfriendGroup.members])
	...
	function startSong():Void
	{
		...
		{
			songPosBar = new FunkinBar(0, 14, "healthBar", FlxColor.GRAY, FlxColor.LIME, LEFT_TO_RIGHT, 'songPositionBar', 0, songLength - 1000);
			songPosBar.bar.numDivisions = 1000;
			songPosBar.screenCenter(X);
			songPosBar.scrollFactor.set();
			songPosBar.alpha = 0;
			#if sys
			for (script in scripts)
				script.setValue("songPosBar", songPosBar);
			#end
			add(songPosBar);

			var songName = new FlxText(0, songPosBar.bg.y + 2, FlxG.width, SONG.song, 16);
			songName.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			songName.scrollFactor.set();
			songName.alpha = 0;
			#if sys
			for (script in scripts)
				script.setValue("songName", songName);
			#end
			add(songName);

			for (shittyShit in [songPosBar, songName])
			{
		...
		DiscordClient.changePresence(detailsText + " " + SONG.song + " (" + storyDifficulty + ") " + generateRanking() + " " + scoreTxt.text.replace("|",
			"\n"), iconRPC);
		#end
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:Strum = new Strum(0, strumLine.y, "NOTE_assets", i, [
			...
			switch (player)
			{
				case 0:
					...
					babyArrow.addOffset("confirm", 50, 50);
			...
			#if sys
			for (script in scripts)
				script.callFunction("generateStrum", [babyArrow, player]);
			#end
		}
	}
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			DiscordClient.changePresence("PAUSED on " + SONG.song + " (" + storyDifficulty + ") " + scoreTxt.text.replace("|", "\n"), iconRPC);
			#end
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.NINE)
		{
			if (iconP1.char == 'bf-old')
				iconP1.changeChar(boyfriendGroup.members[0].iconImage, true);
			else
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			if (FlxG.random.bool(0.1))
			{
				...
				openSubState(new states.substates.PauseSubState(boyfriendGroup.members[0].getScreenPosition().x,
					boyfriendGroup.members[0].getScreenPosition().y));
		}
		...
		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.bar.percent, 0, 100, 100, 0) * 0.01) - iconOffset);
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.bar.percent, 0, 100, 100, 0) * 0.01)) - (iconP2.width - iconOffset);
		...
		if (healthBar.bar.percent < 20)
			iconP1.animation.curAnim.curFrame = 1;
		else if (healthBar.bar.percent > 80 && iconP1.hasWinning)
			iconP1.animation.curAnim.curFrame = 2;
		else
			iconP1.animation.curAnim.curFrame = 0;

		if (healthBar.bar.percent > 80)
			iconP2.animation.curAnim.curFrame = 1;
		else if (healthBar.bar.percent < 20 && iconP2.hasWinning)
		...
		{
			FlxG.switchState(new states.editors.CharEditor(SONG.player1));
			if (lua != null)
			{
				Lua.close(lua);
				lua = null;
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (allowedToHeadbang)
			{
				...
				if (gfGroup.members[0].animation.curAnim.name == 'danceLeft'
					|| gfGroup.members[0].animation.curAnim.name == 'danceRight'
					|| gfGroup.members[0].animation.curAnim.name == 'idle')
				{
								...
								if (curBeat < 250)
								{
									...
									if (curBeat != 184 && curBeat != 216)
									{
										if (curBeat % 16 == 8)
										{
											...
											if (!triggeredAlready)
											{
												gfGroup.members[0].playAnim('cheer');
			...
			if (dadGroup.members[0] != null
				&& dadGroup.length > -1
				&& camFollow.x != dadGroup.members[0].getMidpoint().x + 150
				&& !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				camFollow.setPosition(dadGroup.members[0].getMidpoint().x + 150 + dadGroup.members[0].camPositions[0],
					dadGroup.members[0].getMidpoint().y - 100 + dadGroup.members[0].camPositions[1]);

				switch (dadGroup.members[0].curCharacter)
				{
					case 'mom':
						camFollow.y = dadGroup.members[0].getMidpoint().y;
					case 'senpai':
						camFollow.y = dadGroup.members[0].getMidpoint().y - 430;
						camFollow.x = dadGroup.members[0].getMidpoint().x - 100;
					case 'senpai-angry':
						camFollow.y = dadGroup.members[0].getMidpoint().y - 430;
						camFollow.x = dadGroup.members[0].getMidpoint().x - 100;
				}

				if (dadGroup.members[0].curCharacter == 'mom')
					vocals.volume = 1;
			...
			if (boyfriendGroup.members[0] != null
				&& boyfriendGroup.length > -1
				&& PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection
				&& camFollow.x != boyfriendGroup.members[0].getMidpoint().x - 100)
			{
				camFollow.setPosition(boyfriendGroup.members[0].getMidpoint().x - 100 - boyfriendGroup.members[0].camPositions[0],
					boyfriendGroup.members[0].getMidpoint().y - 100 + boyfriendGroup.members[0].camPositions[1]);
				switch (curStage)
				{
					case 'limo':
						camFollow.x = boyfriendGroup.members[0].getMidpoint().x - 300;
					case 'mall':
						camFollow.y = boyfriendGroup.members[0].getMidpoint().y - 200;
					case 'school':
						camFollow.x = boyfriendGroup.members[0].getMidpoint().x - 200;
						camFollow.y = boyfriendGroup.members[0].getMidpoint().y - 200;
					case 'schoolEvil':
						camFollow.x = boyfriendGroup.members[0].getMidpoint().x - 200;
						camFollow.y = boyfriendGroup.members[0].getMidpoint().y - 200;
		...
		if (health <= 0)
		{
			...
			openSubState(new states.substates.GameOverSubstate(boyfriendGroup.members[0].getScreenPosition().x,
				boyfriendGroup.members[0].getScreenPosition().y));

			#if windows
			// Game Over doesn't get his own variable because it's only used here
			DiscordClient.changePresence("GAME OVER -- " + SONG.song + " (" + storyDifficulty + ") " + generateRanking() + scoreTxt.text, iconRPC);
			#end
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && daNote.cpuCanHit)
				{
					...
					if (daNote.noteType == "altNote" || SONG.notes[Math.floor(curStep / 16)] != null)
					{
						if (daNote.noteType == "altNote" || SONG.notes[Math.floor(curStep / 16)].altAnim)
							altAnim = '-alt';
					}

					for (dad in dadGroup.members)
					{
						if (dad.autoPlayAnims){
					...
					&& daNote.mustPress)
				{
					...
					{
						if (theFunne && daNote.hurtWhenMiss)
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		#if sys
		for (script in scripts)
			script.callFunction("popUpScore", [daNote]);
		#end
		...
		switch (daRating)
		{
			...
			case 'sick':
				...
				addNoteSplash(daNote, playerStrums);
		}
	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && generatedMusic && generatedMusic)
		{
			repPresses++;
			boyfriendGroup.members[0].holdTimer = 0;
			...
			&& generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				if (daNote.canBeHit && daNote.mustPress && daNote.isSustainNote && daNote.canBeHit && daNote.isSustainNote)
				{
					switch (daNote.noteData)
					{
		...
		for (char in [boyfriendGroup.members])
			for (boyfriend in char)
			{
		...
		playerStrums.forEach(function(spr:Strum)
		{
			switch (spr.ID)
			{
				case 2:
					...
					{
						if (upP && spr.animation.curAnim != null && spr.animation.curAnim.name != 'confirm' && !loadRep)
						{
				...
				case 3:
					...
					{
						if (rightP && spr.animation.curAnim != null && spr.animation.curAnim.name != 'confirm' && !loadRep)
							spr.playAnim('pressed');
						if (rightR)
						{
							spr.playAnim('static');
				...
				case 1:
					...
					{
						if (downP && spr.animation.curAnim != null && spr.animation.curAnim.name != 'confirm' && !loadRep)
							spr.playAnim('pressed');
						if (downR)
						{
							spr.playAnim('static');
				...
				case 0:
					...
					{
						if (leftP && spr.animation.curAnim != null && spr.animation.curAnim.name != 'confirm' && !loadRep)
							spr.playAnim('pressed');
						if (leftR)
						{
							spr.playAnim('static');
	...
	function botShit()
	{
		notes.forEachAlive(function(daNote:Note)
		{
			if (daNote.canBeHit && daNote.y > playerStrums.members[daNote.noteData].y - 1500)
			{
				goodNoteHit(daNote, true);
				playerStrums.members[daNote.noteData].playAnim("confirm", true);
			}
		});
	}
	...
	function goodNoteHit(daNote:Note, resetMashViolation = true):Void
	{
		if (!daNote.hurtWhenHit)
		{
			var noteDiff:Float = Math.abs(daNote.strumTime - Conductor.songPosition);
	...
	function updateTrainPos():Void
	{
		...
		{
			startedMoving = true;
			gfGroup.members[0].playAnim('hairBlow');
		}
	...
	{
		gfGroup.members[0].playAnim('hairFall');
	...
	function lightningStrikeShit():Void
	{
		...
		for (character in [boyfriendGroup.members[0], gfGroup.members[0]])
	...
	override function beatHit()
	{
		...
		for (character in [boyfriendGroup.members, dadGroup.members])
			for (char in character)
			{
				if (char != null && char.animation.curAnim != null && !char.animation.curAnim.name.contains("sing"))
					char.dance();
			}

		if (curBeat % 8 == 7 && curSong == 'Bopeebo')
		{
			boyfriendGroup.members[0].playAnim('hey', true);
		}

		if (curBeat % 16 == 15
			&& SONG.song == 'Tutorial'
			&& dadGroup.members[0].curCharacter == 'gf'
			&& curBeat > 16
			&& curBeat < 48)
		{
			boyfriendGroup.members[0].playAnim('hey', true);
			dadGroup.members[0].playAnim('cheer', true);
		}
	...
	function addNoteSplash(daNote:Note, strum:FlxTypedGroup<Strum>)
	{
		if (!daNote.isSustainNote)
		{
			var splash = new NoteSplash(strum.members[daNote.noteData].x - 80, strum.members[daNote.noteData].y - 80, daNote.noteData);
			splash.cameras = [camHUD];
			add(splash);
		}
	}

	function executeEvent(event:String, value1:String, value2:String)
	{
		#if sys
		for (script in scripts)
			script.callFunction("executeEvent", [event, value1, value2]);
		#end
		...
		switch (event)
		{
			case "changeCharacter":
				...
				{
					case "dad":
						dadGroup.members[0].kill();
						dadGroup.remove(dadGroup.members[0]);

						var dad = new Character(100, 100, value2);
						dad.x += dad.positions[0];
						dad.y += dad.positions[1];
						dadGroup.add(dad);
				}
	...
	function luaSetUp()
	{
		if (executeModchart) // dude I hate lua (jkjkjkjk)
		{
			...
			if (result != 0)
				FlxG.game.stage.window.alert("there is an issue with ur Lua Script\n" + getLuaErrorMessage(lua));
