...
class PlayState extends MusicBeatState
{
	...
	private var vocalsBF:FlxSound;
	private var vocalsDad:FlxSound;
	...
	var songHasCutscene:Bool = false;
	...
	override public function create()
	{
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + storyDifficultyText + (Main.watermarks ? " - KE+ " + MainMenuState.kadeEngineVer : ""), 16);
		...
		iconP1 = new HealthIcon(SONG.player1, true, boyfriend.cutIconPrefix);
		iconP1.y = healthBar.y - (iconP1.height / 2);
		add(iconP1);

		iconP2 = new HealthIcon(SONG.player2, false, dad.cutIconPrefix);
		iconP2.y = healthBar.y - (iconP2.height / 2);
		add(iconP2);

		if (FlxG.save.data.colorhp)
			healthBar.createFilledBar(CoolUtil.dominantColor(iconP2), CoolUtil.dominantColor(iconP1));
	...
	function startSong():Void
	{
		...
		if (KadeEngineData.KEOptions.get("songpos"))
		{
			...
			songPosBar = new FlxBar(songPosBG.x + 4, songPosBG.y + 4, LEFT_TO_RIGHT, Std.int(songPosBG.width - 8), Std.int(songPosBG.height - 8), this, "songPos", 0, FlxG.sound.music.length);
			songPosBar.scrollFactor.set();
			songPosBar.createFilledBar(FlxColor.GRAY, FlxColor.LIME);
			add(songPosBar);
